...
export function setupFsHandlers() {
    ...
    // 파일 읽기 (텍스트)
    ipcMain.handle('fs:readText', async (_e, filePath: string, encoding: BufferEncoding = 'utf-8') => {
        const data = await fsp.readFile(filePath, { encoding })
        return data
    })

    // 파일 쓰기 (텍스트) — 상위 폴더 자동 생성
    ipcMain.handle('fs:writeText', async (_e, filePath: string, content: string, encoding: BufferEncoding = 'utf-8') => {
    ...
    // 파일/폴더 삭제
    ipcMain.handle('fs:rm', async (_e, targetPath: string, opts?: { recursive?: boolean; force?: boolean }) => {
        await fsp.rm(targetPath, { recursive: opts?.recursive ?? false, force: opts?.force ?? false })
        return true
    })
    ...
    ipcMain.handle('fs:safeWriteText', async (_e, baseDir: string, relativePath: string, content: string) => {
