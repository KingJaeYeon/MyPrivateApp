...
import { z } from 'zod';
import type { VideoRow } from '@/service/youtube'; // 경로 확인 필요

// ✅ 검증/정규화용 Zod 스키마(숫자/불리언 타입)
export const FilterSchema = z
  .object({
    mode: z.enum(['channels', 'keywords', 'popular']),
...
export type FilterData = {
  mode: 'channels' | 'keywords' | 'popular';
  keyword: string;
  days: string;
  maxChannels: string;
    ...
    persist(
      (set, get) => ({
        ...
        savedHistory: async () => {
          ...
          const history = await window.pref.get('youtubeHistory');
          const next = Array.isArray(history) ? [entry, ...history] : [entry];
          while (next.length > 20) next.pop();
          await window.pref.set('youtubeHistory', next);
        },
        fieldErrorsKeys: [],
        setErrors: (errs) => {
