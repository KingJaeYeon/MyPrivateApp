  ...
  return (
    <div className={'flex flex-3 flex-col'}>
      ...
      <div className={'flex flex-col'}>
        {inputs.map((input, i) => {
          return (
            <div className={'mb-3 flex gap-2'} key={i}>
              <Input
                value={input.idx}
                type={'number'}
                placeholder={'idx'}
                className={'w-[150px]'}
                onChange={(e) => setValues(i, 'idx', e.target.value.trim())}
              />
              <Input
                value={input.name}
                placeholder={'태그명'}
                onChange={(e) => setValues(i, 'name', e.target.value.trim())}
              />
              <Button size={'icon'} disabled={inputs.length === 1} onClick={() => removeInput(i)}>
                <IconTrash />
              </Button>
            </div>
          );
